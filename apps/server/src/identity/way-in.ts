 ...
 * is a key, each carries the same weight, and either opens the same door. They were once two
...
export const WAY_KINDS = ['email', ...PROVIDERS] as const

export type WayKind = (typeof WAY_KINDS)[number]

/** One key. `subject` is the address, or the provider's own id for somebody — never their address. */
export type Key = {
 ...
 */
export function canonical(key: Key): Key {
  return key.kind === 'email' ? { kind: 'email', subject: normalizeEmail(key.subject) } : key
}

/**
 * What a stranger can use here, which is a different question from what an account holds: nobody
 * has any keys yet, and the emailed code is offered to everyone because anyone can prove an
 * address. Only a provider this deployment has keys for is worth showing.
 */
export function offeredWays(offered: Iterable<Provider>): readonly WayKind[] {
 ...
 * it is something to connect, and there is no third state to invent.
 */
export type Way =
...
/**
 * Read, never stored. What is true is which keys this account holds; this is a reading of that,
 * so there is no "which ways does this account support" state that could drift from the rows.
 *
 * Addresses are listed one by one rather than folded into a single "emailed code" line. Folded,
 * nobody can see how many keys there are — and how many keys there are is the entire reason this
 * screen exists.
 *
 * A provider this deployment has no keys for is not offered, because that door opens onto an
 * error. One already connected still shows even if the keys were taken away, since taking it off
 * the list would leave somebody wondering how they got in.
 */
export function waysIn(held: readonly Key[], offered: Iterable<Provider>): readonly Way[] {
  const linked = new Set(held.map((key) => key.kind))
  const available = new Set(offered)

  return [
    ...held
      .filter((key) => key.kind === 'email')
      .map((key) => ({ kind: 'email', address: key.subject, state: 'ready' }) as const),
