...
**「它的机器不在了」是派生的,可它还是得把父任务放出来。** 一件事在等它开出去的活,而那台机器
再也不回来了 —— 派生不派生,父任务都被挡在那儿,永远。所以那条「有没结束的子任务就不派活」的
规则,多一个例外,和「子任务在等你」那个例外一模一样:

```
不派活   有没结束的子任务,除了 —— 在等你的(它在问我)
                              机器不在的(它不会再动了)
```

**两个例外是同一条道理:一个不会再自己往前走的子任务,不是「还在跑」,是一件父任务该知道的事。**

放出来只是一半。父任务还得**被告知**,不然它拿到一轮却不知道为什么:

```
waker 每一轮多问一句   哪些没结束的子任务,机器已经不在,而它爹还没被告诉过?
                       → 往父对话里写一条 handed-back:「你等的那件事的机器不在了」
                       → 叫醒父任务的机器
                       幂等键 gone/<子任务 id>,所以只写一次
```

**两半都要**,和上面那个例外一样 —— 只放不告诉,父 agent 得等到下一次轮询才莫名其妙地醒;
只告诉不放,它醒了也拿不到那一轮。这两半分开写过一次,结果就是两边互相等,而且没有任何东西会说一声。

**顶层的那件事不走这条路。** 它的机器不在了,页面上写着,人自己看得见 —— 恢复方式是等机器回来
或者收回,`prd.md` 那张表就是这么写的。没有第二个人要被告知,所以没有第二件事要做。
...
**命令叫 `task`,而人看到的每一处都叫「一件事」/ piece of work —— 这不是漏网,是定下来的。**
这条命令的读者是 agent,不是人:`task` 在 agent 生态里就是通用词,它看到 `handover task` 不会
困惑,而 `--help` 第一行本来就把话接上了(_the piece of work you were handed_)。改名是一次
破坏性变更,给所有已经写死这条命令的脚本和 prompt 添麻烦,换来的是零个真实的困惑。

**库里的 `tasks` 表和 `task_id` 同理留着** —— 库里的词不是读者的词。
...
`done` + `taken-back`。每一件先写确定命名的 `stop` 活动,再写 `taken-back`,最后叫醒机器;
已经开着的 turn 因此收到真正的停止请求,不是只从待办里消失后继续改文件。
