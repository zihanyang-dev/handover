...
```
邀请 mina 进 Acme
https://…/join/7f3a…                      [ 复制 ] [ 作废 ]
这条链接谁拿到都能进。不用了就把它作废。

一个 Space 任何时候最多只有一条还没作废的邀请链接。关掉 Settings 再回来,只能看见「邀请链接在用」,
不能拿回明文；要再发一次就**换一条**。换一条在同一笔事务里先作废旧链接、再生成新链接,所以旧链接
立刻不能用了,也不会出现两条同时有效的缝。
```
...
`02` 已经把两种动作分开:**Space 页面说 Share / Stop sharing;彻底 Disconnect 只在机器主人的
Account 里。** 那是他的电脑,你能在这个 Space 用,但拿不走。停止分享前先列出它仍承载的 work,
清单不为空就先处理,所以这不是一个看不见后果的移除动作。
...
她添加给这里的机器会从 Acme 移除
  build-server-1           机器本身和其它 Space 不受影响
  mina-mbp                 机器本身和其它 Space 不受影响
...
所以移除给的不是一个动作,是**一份要处理的 work 清单**。机器不是责任转移:移除成员会撤销他在这个
Space 的机器关系,不会全局断开他的电脑,也不会影响其它 Space。
...
```
① 一条链接就能请人进来。一个 Space 同时最多一条;链接能换、能作废、会过期,明文只给你看一次
...
⑨ 他的成员凭据当场失效;他添加给这个 Space 的机器关系撤销,机器本身和其它 Space 不受影响
```
...
```
生成一条邀请链接 → 明文只出现一次,关掉再看只知道它在用
换一条邀请链接 → 旧链接和新链接不同时有效;旧链接立刻不能用,新链接明文出现一次
把链接发给第二个人,他点开 → 没登录先登录,登录完回到这条链接
他点加入 → 进到这个 Space,看到同样的机器和对话
他把自己的机器分享给这个 Space → 第一个人看得见,而且标着是他的
第一个人想断开他的机器 → 没有那个按钮
把链接作废 → 再点就说这条邀请不能用了
链接过期 → 同上,措辞不同
member 想邀请人 → 没有那个入口
...
移除一个人 → 先看到他名下 2 件在跑的事;他的机器会从当前 Space 移除
把一件事转给另一个人 → 那件事出现在新主人的 Inbox 里
移除之后 → 他的会话立刻不能用了,机器仍在他的 Account 和其它 Space,他说过的话仍写着他的名字
把同一个人再请进来 → 他原来的东西都还在
```
