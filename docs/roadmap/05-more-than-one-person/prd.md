...
> **除了「换主人」,这一片做完了。**旅程 ①②③④⑤⑦⑧⑨ 全走通,两个人在真浏览器里走过一遍
> (`e2e/joining.spec.ts`)。**⑥ 的清单在,清单上的 `[ 转给… ]` 不在** —— 见下。
...
> **现在到这里为止:清单看得见,`[ 转给… ]` 还没有。**按下移除之前,他名下在跑的事和他的机器
> 都列在眼前,并且屏幕明说「这里的东西不会自己停,也不会自己搬」。要处理其中一件,从清单点进
> 那段对话去处理。
>
> 换主人没做,不是因为难,是因为它卡在一条**还没决定的 schema** 上:`machines` 现在被复合外键
> 钉在 `enrolments.approved_by`(「机器不能和批准它的人不一致」),换主人要么连着改批准记录 ——
> 而批准记录是历史,历史不该被改 —— 要么把那条外键换成一条只说「主人存在」的约束,而那会丢掉
> 一条现在拦得住事情的规矩。**先定这件事,再写代码**(`design.md`「要动的现有东西」)。
