...
该有的样子。超过 8000 字节 Postgres 会拒绝,所以输出带 `callId + at`,并按 UTF-8 **字节**切成
`PIECE` 以内的片段;`at` 仍是浏览器字符串使用的 UTF-16 offset。CLI 以 75ms 的短周期合并
provider 的高频更新,浏览器每个调用最多留 256 KiB,超过就明确写 `Earlier output unavailable`。
Codex app-server 对极快命令可能来不及暴露第一个 stdout chunk;adapter 不猜那段内容,而是在 live
片段和 durable tool excerpt 上保留 `truncated`,同样显示 `Earlier output unavailable`。这些界限控制
的是通知频率和内存,**没有一片输出进 messages 表**;「到第 N 条了」仍只是一个数字。

机器的 live POST 在同一个事务里锁住机器、重查它没有被移除且对话确实属于它,再 `NOTIFY`;
人的 typing 每次确认对话确实在路径里的 Space。
SSE 打开后也不是一次授权管到底:每一帧发送前重新确认成员仍在,被移除就关闭流。瞬时内容不落库,
不等于它没有权限边界。
