...
```
线上            屏幕上             为什么不统一
invitations     link              线上那个是一行可作废、会过期的记录;人手里拿着的是一条 URL
...
不写下来的代价是真的:写 People 那一屏时,`invitations` 到 "link" 是现场翻的,而下一个人会再翻一次,
并且可能翻成别的词。
...
- **默认用 query builder** `test` —— 它的类型是从活 schema 生成的,列改了编译就断。
  ...
  两条都由 `rules/sql.spec.ts` 强制:**允许出现的整条语句是一张手改的名单**,加一条得有人特意去改。
